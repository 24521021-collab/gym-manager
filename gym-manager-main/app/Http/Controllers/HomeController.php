...
class HomeController extends Controller
{
    ...
    public function index() {
        ...
        $catalogRecommendations = FitnessCatalogRecommendationService::forBmi($latestMetric?->bmi ? (float) $latestMetric->bmi : null);
        $recommendedPackageIds = $catalogRecommendations['packages']->pluck('id')->all();
        $recommendedClasses = $catalogRecommendations['classes'];
        $goiTaps = GymPackage::paginate(8);
        // Trả về view 'trangchu' và truyền các biến `$goiTaps`, `$latestMetric`, `$posts` sang view để hiển thị.
        return view('trangchu', compact('goiTaps', 'latestMetric', 'posts', 'recommendedPackageIds', 'recommendedClasses'));
    }  
}
