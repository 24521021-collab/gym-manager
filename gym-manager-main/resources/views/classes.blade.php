...
<script>
    ...
    let currentSearch = '';
    const recommendedClassId = Number(new URLSearchParams(window.location.search).get('recommended_class') || 0);
    let searchTimer;
    ...
    function renderClasses() {
        ...
        filtered.forEach(c => {
            ...
            const html = `
                <div id="class-card-${c.id}" class="class-item bg-gradient-to-b from-[#1E1E1E] to-[#141414] border ${recommendedClassId === c.id ? 'border-primary ring-2 ring-primary shadow-[0_0_35px_rgba(227,24,55,0.45)]' : 'border-white/10'} rounded-2xl overflow-hidden shadow-2xl hover:border-primary/40 transition-all duration-300 flex flex-col group relative">
                    ${recommendedClassId === c.id ? '<span class="absolute top-4 left-4 z-20 text-[10px] font-bold bg-primary text-white px-3 py-1 rounded-md uppercase tracking-wider">Phu hop voi chi so</span>' : ''}
        ...
        if (recommendedClassId) {
            const recommendedCard = document.getElementById(`class-card-${recommendedClassId}`);
            if (recommendedCard) {
                setTimeout(() => recommendedCard.scrollIntoView({ behavior: 'smooth', block: 'center' }), 150);
            }
        }
    }
    /**
