                    ...
                    <div class="bg-black/30 border border-white/10 rounded-xl p-4">
                        ...
                        <div id="recommended-packages-list" class="space-y-2 text-xs text-gray-300">
                            @forelse(($recommendedPackageIds ?? []) as $recommendedPackageId)
                                @php $recommendedPackage = collect($goiTaps->items())->firstWhere('id', $recommendedPackageId); @endphp
                                @if($recommendedPackage)
                                    <div class="flex justify-between gap-3 border border-white/5 rounded-lg px-3 py-2 bg-white/5">
                                        <span class="font-bold text-white">{{ $recommendedPackage->package_name }}</span>
                                        <span class="text-primary font-mono">{{ number_format($recommendedPackage->price, 0, ',', '.') }}đ</span>
                                    </div>
                                @endif
                            @empty
...
<script>
    const initialRecommendedPackageIds = @json($recommendedPackageIds ?? []);

    function highlightRecommendedPackages(packageIds = []) {
        document.querySelectorAll('.btn-register').forEach(button => {
            const card = button.closest('.group');
            if (!card) return;

            card.classList.remove('ring-2', 'ring-primary', 'shadow-[0_0_35px_rgba(227,24,55,0.45)]');
            const oldBadge = card.querySelector('.js-recommended-badge');
    ...
    function buildLocalRecommendation(bmi, bodyFat) {
    ...
    function renderCatalogRecommendations(catalog) {
        const packageBox = document.getElementById('recommended-packages-list');
        const classBox = document.getElementById('recommended-classes-list');
        if (!packageBox || !classBox || !catalog) return;

        const packages = catalog.packages || [];
        const classes = catalog.classes || [];

        packageBox.innerHTML = packages.length
            ? packages.map(item => `
                <div class="flex justify-between gap-3 border border-white/5 rounded-lg px-3 py-2 bg-white/5">
    ...
    function updateHealthMetricsDisplay(height, weight, bodyFat, serverRecommendation = null) {
        ...
        if (bmi !== '--' && !isNaN(bmi)) {
            ...
            panel.className = "p-4 rounded-xl border transition-all duration-300 flex flex-col md:flex-row justify-between items-start md:items-center gap-4";
            if (recommendation.risk_level === 'danger') {
                panel.classList.add('border-red-500/30', 'bg-red-500/5');
            } else if (recommendation.risk_level === 'warning') {
                panel.classList.add('border-yellow-500/30', 'bg-yellow-500/5');
            } else if (recommendation.risk_level === 'info') {
                panel.classList.add('border-blue-500/30', 'bg-blue-500/5');
            } else {
                panel.classList.add('border-green-500/30', 'bg-green-500/5');
            }
        }
    ...
    document.getElementById('update-metrics-btn').addEventListener('click', async function() {
        ...
        try {
            ...
            if (response.ok) {
                ...
                updateHealthMetricsDisplay(height, weight, bodyFat, data.recommendation);
                renderCatalogRecommendations(data.recommendation?.catalog);
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        highlightRecommendedPackages(initialRecommendedPackageIds);
        setStarRating(5); 
